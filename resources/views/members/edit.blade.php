            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('members.update', $member) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $member->name) }}" class="mt-1 block w-full border rounded px-3 py-2" required>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $member->phone) }}" class="mt-1 block w-full border rounded px-3 py-2" required>
                            @error('phone')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">NID</label>
                            <input type="text" name="nid" value="{{ old('nid', $member->nid) }}" class="mt-1 block w-full border rounded px-3 py-2" required>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email', optional($member->user)->email) }}" class="mt-1 block w-full border rounded px-3 py-2" required>
                        ...
                        <div>
                            ...
                            <select name="member_type" class="mt-1 block w-full border rounded px-3 py-2" required>
                                @foreach(['admin'=>'Admin','accountant'=>'Accountant','member'=>'Member'] as $k=>$v)
                                    <option value="{{ $k }}" @selected(old('member_type', $member->member_type)===$k)>{{ $v }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="3" class="mt-1 block w-full border rounded px-3 py-2">{{ old('address', $member->address) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="join_date" value="{{ old('join_date', optional($member->join_date)->toDateString()) }}" class="mt-1 block w-full border rounded px-3 py-2" required>
                        ...
                        <div>
                            ...
                            <select name="status" class="mt-1 block w-full border rounded px-3 py-2" required>
                                @foreach(['active'=>'Active','inactive'=>'Inactive','suspended'=>'Suspended'] as $k=>$v)
                                    <option value="{{ $k }}" @selected(old('status', $member->status)===$k)>{{ $v }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="mt-3 flex items-center gap-4">
                            @if($member->profile_picture)
                                <img src="{{ asset('storage/'.$member->profile_picture) }}" class="h-24 w-24 rounded object-cover" alt="Current" />
                            @endif
                ...
                <div class="mt-8">
                    <details>
                        <summary class="cursor-pointer text-sm text-gray-600">Admin Reset Password</summary>
                        <form action="{{ route('members.reset-password', $member) }}" method="POST" class="mt-2 space-y-2">
                            @csrf
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="password" name="new_password_confirmation" class="mt-1 w-full border rounded px-3 py-2" required>
                                </div>
                            </div>
                            @error('new_password')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
                            <button class="px-4 py-2 text-sm bg-amber-600 text-white rounded" type="submit">Reset Password</button>
                        </form>
                    </details>
                </div>
                @endif
